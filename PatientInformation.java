...
public class PatientInformation extends JFrame {
    ...
    private final PharmacyLinearSearch linearSearch;

    public PatientInformation() {
        pharmacy = new PharmacyAlgorithm();
        linearSearch = new PharmacyLinearSearch();
        ...
        titleBar.setPreferredSize(new Dimension(960, 40));
...
//        titleLeftPanel.add(logoLabel);
        ...
        searchButton.addActionListener(e -> {
            ...
            // Measure ONLY the linear search (no initialization overhead)
            long linearStartTime = System.nanoTime();
            linearSearch.linearSearchWithRuntime(condition);
            long linearEndTime = System.nanoTime();
            double linearRuntime = (linearEndTime - linearStartTime) / 1_000_000.0;

            // Measure ONLY the HashMap search (no initialization overhead)
            long startTime = System.nanoTime();
            currentMedicines = pharmacy.findMedicineByCondition(condition);
            long endTime = System.nanoTime();
            double hashRuntime = (endTime - startTime) / 1_000_000.0;

            // Display HashMap runtime
            algoRuntimeField.setText(String.format("%.4f ms", hashRuntime));

            // Console output with speedup calculation
            System.out.println("======================================================");
            System.out.println("--SEARCHING RUNTIME--");
            System.out.printf(" Linear Search + ArrayList: %.6f ms%n", linearRuntime);
            System.out.printf(" HashMap + Hashing:         %.6f ms%n", hashRuntime);
            System.out.println("======================================================");
    ...
    private void warmUpSearches() {
        pharmacy.findMedicineByCondition("Headache");
        pharmacy.findMedicineByCondition("Fever");
        pharmacy.findMedicineByCondition("Cough");

        linearSearch.linearSearchWithRuntime("Headache");
        linearSearch.linearSearchWithRuntime("Fever");
        linearSearch.linearSearchWithRuntime("Cough");
    }
    ...
    private void setupAutocomplete(JButton searchButton) {
        ...
        // Add key listener to suggestion list for navigation
